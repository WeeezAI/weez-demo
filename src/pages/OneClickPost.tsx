...
const OneClickPost = () => {
  ...
  const [selectedModel, setSelectedModel] = useState<"gpt-image-2" | "imagen4" | "gpt-image" | "flux-pro" | "flux-flex">("gpt-image-2");
  ...
  const handleEdit = async () => {
    if (!generatedCreative || !editPrompt.trim()) return;
    setIsEditing(true);
    try {
      const brandId = spaceId || "test-brand-123";
      const result = await weezAPI.editPoster(brandId, generatedCreative.blob_name, editPrompt, "1:1");
      ...
      setEditPrompt("");
      toast.success("Visual adjustment complete");
    } catch (error) {
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedModel === "gpt-image-2"
                      ? "bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-300 shadow-lg shadow-emerald-500/10 ring-2 ring-emerald-400/30"
                      : "bg-white border-border/50 hover:border-emerald-200 hover:bg-emerald-50/30"
                  )}
                >
                  ...
                  <div className="text-left">
                    <div className={cn(
                      ...
                      selectedModel === "gpt-image-2" ? "text-emerald-700" : "text-foreground"
                    )}>
                      GPT-Image-2
                    </div>
                    <div className="text-[9px] text-muted-foreground mt-0.5">🏆 Best Quality</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedModel === "imagen4"
                      ? "bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-300 shadow-lg shadow-blue-500/10 ring-2 ring-blue-400/30"
                      : "bg-white border-border/50 hover:border-blue-200 hover:bg-blue-50/30"
                  )}
                >
                  {/* Google Logo SVG */}
                  <div className="w-7 h-7 flex-shrink-0">
                  ...
                  <div className="text-left">
                    <div className={cn(
                      "text-[11px] font-black uppercase tracking-wider",
                      selectedModel === "imagen4" ? "text-blue-700" : "text-foreground"
                    )}>
                      Imagen 4
                    </div>
                    <div className="text-[9px] text-muted-foreground mt-0.5">⚡ Fast</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    <div className={cn(
                      "text-[11px] font-black uppercase tracking-wider",
                      selectedModel === "gpt-image" ? "text-emerald-700" : "text-foreground"
                    )}>
                      GPT-Image
                    </div>
                    <div className="text-[9px] text-muted-foreground mt-0.5">🎨 Artistic</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedModel === "flux-pro"
                      ? "bg-gradient-to-br from-violet-50 to-purple-50 border-violet-300 shadow-lg shadow-violet-500/10 ring-2 ring-violet-400/30"
                      : "bg-white border-border/50 hover:border-violet-200 hover:bg-violet-50/30"
                  )}
                >
                  <div className="w-7 h-7 flex-shrink-0 flex items-center justify-center">
                    <Zap className={cn(
                      "w-5 h-5 transition-colors",
                      selectedModel === "flux-pro" ? "text-violet-600 fill-violet-600" : "text-muted-foreground"
                    )} />
                  </div>
                  <div className="text-left">
                    <div className={cn(
                      "text-[11px] font-black uppercase tracking-wider",
                      selectedModel === "flux-pro" ? "text-violet-700" : "text-foreground"
                    )}>
                      FLUX.2-pro
                    </div>
          ...
          <div className="flex flex-col md:flex-row h-full max-h-[92vh] bg-white rounded-[4rem] overflow-hidden shadow-[0_120px_240px_rgba(139,92,246,0.3)] border border-white/20">
            ...
            <div className="flex-1 p-14 flex flex-col justify-between bg-[#FDFBFF] relative border-l border-border/40 overflow-y-auto">
              <div className="space-y-12">
                ...
                {/* Poster Adjustment Section */}
                {!isGenerating && !hasPosted && (
                  <div className="space-y-6 pt-6 border-t border-border/40">
                    <div className="flex items-center justify-between px-2">
                      <Label className="text-[11px] font-black uppercase tracking-[0.4em] text-primary">Visual Adjustment</Label>
                    ...
                    <p className="text-[9px] font-medium text-muted-foreground/40 px-2 italic uppercase tracking-wider">
                      Uses gpt-image-2 for precision modifications
                    </p>
                  </div>
                )}
              </div>
